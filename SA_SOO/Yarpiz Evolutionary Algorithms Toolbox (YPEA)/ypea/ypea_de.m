...
classdef ypea_de < ypea_algorithm
    ...
    methods
        ...
        function SetParameters(this, parameters)
            this.pop_size = round(parameters(1));
            base_vect_mode = floor(parameters(2));
            switch(base_vect_mode)
                case 1
                    this.base_vector = 'best';
                case 2
                    this.base_vector = 'target-to-best';
            ...
            this.diff_vectors_count = max(round(parameters(3)*parameters(1)),1);
            crossover_mode = floor(parameters(4));
            ...
            this.crossover_prob = parameters(5);
            this.beta_min = parameters(6);
            this.beta_max = parameters(7)+parameters(6);
        end
    end
    ...
    methods(Access = protected)
        ...
        function y = perform_mutation(this, i)
            ...
            % Calculating all of them a single time to avoid overhead)
            beta_size = [ndv, var_size(2)];
            beta = @() ypea_uniform_rand(this.beta_min, this.beta_max, beta_size);
            ...
            random_betas = beta();
            for k = 1:numel(a)
                y = y + random_betas(k,:).*(this.pop(a(k)).position - this.pop(b(k)).position);
            end
