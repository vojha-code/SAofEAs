...
nObj = numel(CostFunction(unifrnd(VarMin, VarMax, VarSize),nProblem));
...
for i = 1:nPop
    ...
    pop(i).Cost = CostFunction(pop(i).Position,nProblem);
end
...
for it = 1:MaxIt
    ...
    for k = 1:nCrossover/2
        ...
        popc(k, 1).Cost = CostFunction(popc(k, 1).Position,nProblem);
        popc(k, 2).Cost = CostFunction(popc(k, 2).Position,nProblem);
    ...
    for k = 1:nMutation
        ...
        popm(k).Cost = CostFunction(popm(k).Position,nProblem);
