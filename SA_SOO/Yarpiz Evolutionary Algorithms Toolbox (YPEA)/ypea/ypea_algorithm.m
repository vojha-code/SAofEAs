...
% What is modified:
% - max_nfe and goal moved here, so one can set them before assigning the
% problem 
% - Using a SetTermination more generic function to decide the termination
% conditions
...
classdef (Abstract) ypea_algorithm < handle
    ...
    methods
        ...
        function check_for_goals(this)
            ...
            if ~isempty(this.goal) && this.is_better(this.best_sol, this.goal)
                this.stop();
            end
            
            % Check if Maximum NFE is reached
            if ~isempty(this.max_nfe) && this.nfe >= this.max_nfe
                this.stop();
            end
            
        end
        
        function SetTermination(this, termination)
            for i=1:floor(numel(termination)/2)
            	switch termination{2*i-1}
                    case "NEvaluations"
                        this.max_nfe = termination{2*i};
                    case "BestSolution"
                        this.goal = termination{2*i};
                    case "MaxIter"
                        if termination{2*i}=='popdependant'
                            this.max_iter = ceil(this.max_nfe/this.pop_size);
                        else
                            this.max_iter = termination{2*i};
                        end
                    otherwise
                        null;
                end
