...
if ~isempty(i_axis) % Show only the input range specified by user
    ...
    for j=1:10; str{j}=num2str(round(10^nb_fig*YTick(j))/(10^nb_fig)); end
    ...
    for i=1:M
        xmin_i=round(10^nb_fig*xmin(i))/(10^nb_fig);
        xmax_i=round(10^nb_fig*xmax(i))/(10^nb_fig);
        text(i+0.05,0,num2str(xmin_i),'HorizontalAlignment','Center','VerticalAlignment','Top','Fontsize',fs,'Fontname',fn)
        text(i+0.05,1,num2str(xmax_i),'HorizontalAlignment','Center','VerticalAlignment','Bottom','Fontsize',fs,'Fontname',fn)
